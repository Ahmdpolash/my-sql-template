...
export const sendEmail = async (
  ...
  await transporter.sendMail({
    from: `"Suiamor" <${config.sendEmail.brevo_email}>`,
    to,
...
export const sendContactEmail = async (to: string, payload: { fullName: string; email: string; subject: string; description: string }) => {
  ...
  await transporter.sendMail({
    from: ` ${payload.fullName} <${config.sendEmail.brevo_email}>`,
...
export const sendBulkEmail = async (
  ...
  subject: string,
  message: string,
  isHtml: boolean = false // true hole HTML, false hole plain text
) => {
  const transporter = nodemailer.createTransport({
    host: "smtp-relay.brevo.com",
    port: 587,
  ...
  // HTML template (optional - customize as needed)
  const htmlTemplate = `
  <div style="max-width: 600px; margin: 0 auto; background-color: #F6F7F9; color: #000; border-radius: 8px; padding: 24px;">
    <table style="width: 100%;">
      <tr>
    ...
    <div style="text-align: center; margin-top: 20px; color: #999; font-size: 12px;">
      &copy; ${new Date().getFullYear()} suiamor. All rights reserved.
    </div>
  </div>
  `;
  ...
  await transporter.sendMail({
    from: `"Suiamor" <${config.sendEmail.brevo_email}>`,
    bcc: recipients, // BCC te sob email address - keu karo email dekhbe na
    subject: subject,
    text: isHtml ? undefined : message, // Plain text message
    html: isHtml ? htmlTemplate : undefined, // HTML message
  });
};
